...
const TS_CONDITION_GUIDE = `condition.clauses[].type로 쓸 수 있는 값과 필드 (이 목록 밖의 type은 절대 쓰지 말 것):
...
function tsIsValidCondition(condition) {
  ...
  return condition.clauses.every(
    (c) => c && typeof c === "object" && TS_ALLOWED_CLAUSE_TYPES.includes(sanitizeStr(c.type))
  );
}
...
async function tsCreateStrategy(user, body, res) {
  ...
  if (!stockCode || !stockName || !rawText || !tsIsValidCondition(condition)) {
