...
const BEGINNER_RULE = [
  ...
  "- watch(앞으로 이것만 보세요): 정확히 3개, 높은 가격부터 낮은 가격 순. price는 이 리포트에 나온 실제 가격(시나리오 조건·저항·지지·이동평균) 중 하나를 숫자로, text는 '무슨 일이 생기면 → 무엇을 확인'을 45자 이내로(예: '이 가격을 넘고 거래량이 늘면 → 상승 추세가 강해지는지 확인'). 시나리오 A/B/C 조건과 같은 가격을 쓴다. 가격은 price 칸에 따로 표시되니 text에는 가격 숫자를 다시 쓰지 않는다('이 가격을 넘고…'처럼). 확률 숫자는 쓰지 않는다.",
...
/** 사용자 제보 — "다가오는 이벤트"에 이미 지나간 뉴스(예: 이미 발표된 실적)가 그대로
 * 섞여 나온 사례가 있었다. isPastEventDate()는 date 필드에 깔끔한 ISO 날짜가 박혀
 * 있을 때만 걸러내는데, 실제 사고 사례는 date 없이 "최근 분기 실적 발표 이후 ~"처럼
 * 서술형 문장만 있는 경우라 그 필터를 그냥 통과했다. '예정/계획' 같은 미래 표현이
 * 전혀 없이 과거형 어미로 끝나는 문장이면 실제로는 이미 벌어진 일을 이벤트로 위장한
 * 것으로 보고 걸러낸다. */
function isPastLeaningEventContent(content) {
  const c = String(content || "").trim();
  if (!c) return false;
  if (/예정|계획|전망|앞두|다가오는|개최될|열릴|출시될|공개될|시행될|D-\d+/.test(c)) return false;
  return /(했다|됐다|였다|았다|었다|밝혔다|기록했다|발표했다|공시했다|완료했다|마쳤다|끝났다|보였다|나타났다)\s*[.]?\s*$/.test(c);
}

/** 실사용 중 발견 — SK하이닉스 실사용 테스트에서 "다가오는 이벤트"에 "오늘 기준
 * 외국인과 기관이 모두 순매도라서 단기 수급이 추세를 받쳐주지 못하고 있다"(날짜:
 * 오늘, type: 악재)가 들어온 사례를 확인했다. 이 문장은 과거형으로 끝나지 않아서
 * (현재형 "있다") isPastLeaningEventContent를 통과하고, 날짜도 채워져 있어서(오늘
 * 날짜) 앞의 필터들도 통과했지만, 실제로는 "오늘 현재 상황 설명"이지 앞으로 있을
 * 예정된 일정이 아니다 — 수급 분석/재료 분석 섹션과 내용도 그대로 겹친다. "진짜
 * 일정"이라면 (a) 예정/계획 등 미래 지향 표현이 있거나 (b) 날짜 자체가 오늘보다
 * 뒤여야 한다는 두 조건 중 최소 하나는 성립해야 한다는 것을 역으로 이용해, 둘 다
 * 아니면(오늘 이하 날짜 + 미래 표현 없음) 걸러낸다. */
function looksLikeScheduledFutureEvent(content, dateStr, todayISO) {
  const c = String(content || "").trim();
  const hasFutureKeyword = /예정|계획|전망|앞두|다가오는|개최될|열릴|출시될|공개될|시행될|D-\d+|참석/.test(c);
  const iso = extractISODate(dateStr);
  // 사용자 피드백(2026-08-27): 날짜가 명확히 파싱되면 "오늘 날짜"도 제외한다 — 당일 이벤트는
  // 이미 발표·개최가 끝났을 수도 있는데(예: 실적 발표 당일에도 이미 발표된 경우) '예정' 같은
  // 미래 키워드만 믿고 통과시키면 이미 지난 사실을 미래처럼 보여주는 사고가 난다. 날짜가 없거나
  // 파싱 불가능한 모호한 시기('2026년 하반기 예정')일 때만 키워드 기반으로 통과시킨다.
...
- [문체 — 리포트 전체에 반드시 적용] 목표는 "정확한데 세련되고 다정하게 읽히는 글"이다. 사실은 정확하게, 해석은 사람처럼 쓴다.
  · 말투는 세련되고 유연해야 한다 — 딱딱한 보고서체나 사무적인 나열로 읽히면 안 된다. 실력 있는 애널리스트가 옆에서 차분히 설명해주는 느낌으로, 문장 사이가 자연스럽게 이어지도록 쓴다. 모든 문장을 짧게 뚝뚝 끊기만 하면 오히려 딱딱해진다 — 짧은 문장과 조금 더 풀어 쓴 문장을 섞어 리듬을 만든다.
  · [독자에게 말하듯 쓴다] 결론만 일방적으로 던지지 말고, 독자가 그 대목에서 떠올릴 법한 궁금증·걱정을 짧게 대신 말해준 뒤 답해주는 흐름을 섞는다 — 예: "'주가가 비싸진 게 아닐까?'라는 걱정이 들 수 있지만, ~". 설명은 친절하게, 결론은 분명하게 낸다.
  · [쉬운 설명 — 초보자 배려] 전문용어는 괄호 풀이뿐 아니라 문장 자체를 쉬운 말로 먼저 풀어서 녹인다 — "밸류에이션 부담"보다 "주가가 비싸 보이는 부담", "수급 반전"보다 "다시 돌아온 매수세"처럼 일상적인 표현을 우선하고, 꼭 필요하면 괄호로 원래 용어를 병기한다("눌림목"처럼 대체하기 애매한 용어는 문장 안에 자연스럽게 괄호 설명을 끼워 넣는다: "주가가 잠시 낮아질 때(눌림목)"). 같은 용어를 같은 리포트 안에서 반복 설명하지는 않는다.
  · [좋은 예 — 이런 톤을 목표로 한다] "AI 반도체의 핵심인 HBM 기대감, 하루가 다르게 높아지는 실적 눈높이, 그리고 9월 들어 다시 돌아온 매수세까지 삼박자가 딱 맞물린 시점입니다. 최근 주가가 단기에 빠르게 올라 잠시 흔들릴 수는 있습니다. 하지만 '주가가 비싸진 게 아닐까?'라는 걱정보다, 기업이 실제로 벌어들일 돈(이익)이 늘어나는 속도가 훨씬 빠릅니다. 따라서 지금처럼 치솟는 주식을 조급하게 따라 사기보다는, 주가가 잠시 숨고르기하며 살짝 낮아질 때(눌림목)를 차분히 기다리는 편이 훨씬 낫고 안전합니다."
  · [나쁜 예 — 이렇게 쓰지 않는다] "HBM 기대와 실적 상향, 그리고 9월 들어 확인된 수급 반전이 동시에 붙은 구간이다. 단기 급등 뒤 변동성은 남아 있지만, 밸류에이션 부담보다 이익 추정 상향 속도가 더 빠르다. 오늘은 추격보다 눌림을 보는 쪽이 더 유효하다." (문제: '-다'체라 딱딱하고, 용어를 안 풀고 그대로 썼고, 독자에게 말을 거는 느낌이 없다)
  · 종결은 정중체 '-습니다'로 통일한다('-다'체와 섞지 않는다). 다만 같은 종결 패턴을 연속 두 문장 이상 반복하지 않는다 — '-습니다/-했습니다/-됩니다/-일 것입니다'와 부드러운 연결형("~는데요", "~거든요"는 과함 — 쓰지 않는다)을 남발하지 않으면서도 문장마다 어미를 조금씩 바꿔 리듬을 만든다.
  · 한 문장은 한 가지만 말한다. 문장 길이는 40~80자를 기본으로 하되, 독자에게 말을 거는 흐름을 살리기 위해 필요하면 110자까지는 허용한다(그 이상은 안 됨). '~하고, ~며, ~어서, ~인데'로 절을 넷 이상 이어붙인 장문은 반드시 두세 문장으로 쪼갠다.
  · 긴 서술 필드(story, supply, chart, opinion.comment, opinion.short/mid/long, materials.summary, materials.unreflected)는 한 덩어리로 쓰지 말고 2~4문장마다 빈 줄 하나(\n\n)로 문단을 나눈다. 한 문단에는 논점 하나만 담는다.
  · 각 문단은 결론부터 시작한다(두괄식). 수치·근거는 그 뒤에 붙인다.
  · 아래 상투구는 리포트 전체에서 금지한다 — "~라고 보는 게 맞다", "~로 보는 게 맞다", "~라는 점이 중요하다", "~에 무게가 실린다", "~라고 볼 수 있다", "전형적인 모습이다", "~가 유력하다", "~에 가깝다", "~로 읽힌다", **"정교하다"·"더 정교합니다"**(일상에서 쓰지 않는 말이라 어색하다 — "더 낫습니다", "더 바람직합니다", "이쪽이 안전합니다"처럼 평범한 말로 쓴다). 같은 뜻이라도 매번 다른 표현으로 새로 쓴다.
...
- [섹션 역할 분담 — 반드시 준수] 같은 재료·같은 논거를 세 개 이상 섹션에서 반복하지 않는다(최대 두 곳). 각 섹션의 역할은 이렇게 나눈다.
  ...
  · chart(6번) = 가격 구조.
  · opinion(7번) = 그래서 어떻게 대응할지. 앞 섹션 근거는 한 줄로 압축해 인용만 하고 다시 설명하지 않는다.
- [이미 끝난 이벤트는 재료가 아니다 — 반드시 준수] 발표·집행이 완료돼 주가에 대부분 반영된 과거 이벤트(예: 지난달 끝난 실적발표)는 materials 항목으로 만들지 않는다. reflectionPct가 90% 이상으로 매겨질 건이라면 애초에 재료 슬롯을 쓰지 않는다. 재료 슬롯은 앞으로 주가를 움직일 수 있는 건에만 배정한다. 과거 이벤트가 맥락상 필요하면 story에서 한 문장 배경으로만 언급한다.
- [재료 카드 문장 규칙] 같은 재료의 reflectionBasis(반영도 근거)와 judgment(한줄 판단)에 같은 문장·같은 숫자를 중복해 쓰지 않는다. reflectionBasis는 "왜 그 %인지"의 정량 근거, judgment는 "그래서 투자자에게 무슨 뜻인지"의 해석으로 명확히 나눈다. judgment는 한 문장, 60자 이내로 짧게 끊는다.
...
- upcomingEvents에는 web_search로 날짜와 사실관계가 명확히 확인된 "미래" 이벤트만 포함할 것 — 이미 벌어진 일(지난 실적발표 결과, 이미 나온 뉴스·공시·이슈)은 절대 포함하지 않는다. 그런 내용은 2번 story나 5번 재료 분석(materials)에만 넣을 것 — 이 섹션과 재료 분석 섹션이 같은 내용을 반복하면 안 된다.
- 확인이 안 됐거나 애매한 건은 "아직 확인되지 않음" 같은 문구로 이벤트에 넣지 말고 아예 배열에서 제외할 것 (미확정 사실을 이벤트인 것처럼 만들지 않는다).
- 오늘은 ${today} 이다. 오늘 날짜는 제외하고, 이 날짜보다 뒤(내일 이후)의 미래 이벤트만 포함할 것.
- 2024년, 2025년 날짜는 절대 포함 금지. 2026년 이후만 허용.
- 웹검색 결과에서 '예정' '방문 예정' '출시 예정' '발표 예정' '~할 계획' '~에 참석' 키워드가 있는 것만 선별.
- '했다' '밝혔다' '기록했다' '하락했다' '보였다' '나타났다' 등 과거형으로 끝나는 문장은 절대 이벤트로 넣지 말 것 — "~예정"/"~계획" 같은 미래 표현이 없으면 그건 이벤트가 아니라 이미 벌어진 일이다.
- date가 비어 있는 항목은 넣지 말 것 — 날짜(정확한 날짜든 '2026년 하반기 예정' 같은 대략적 시기든)를 전혀 특정할 수 없으면 이벤트로 넣지 않는다.
- 이벤트는 '앞으로 있을 예정된 일정'이어야 한다 — 오늘/현재 시점의 상황 설명(예: '오늘 수급이 순매도라 추세가 약하다')은 이벤트가 아니라 수급 분석·재료 분석에 속한다.
- 웹검색으로 다음 실적발표일 등 공식 일정이 명확히 확인되면 그것으로 채우고, 확인이 안 되면 무리해서 채우지 말 것.
- 이벤트가 정말 하나도 확인되지 않으면 억지로 만들지 말고 upcomingEvents를 빈 배열로 둘 것 — 화면에는 이미 "현재 확인된 예정 이벤트 없음"이 자동으로 표시되니 가짜 항목을 만들 필요가 없다.
- 이벤트 제목은 구체적으로: ❌'CEO 해외 일정' → ✅'젠슨황 방한 / 삼성 CEO 회동 예정'
- 날짜를 정확히 모르면 '2026년 하반기 예정' 식으로 표현.`,
    "",
    `5번 재료 분석 — web_search 결과 기반 (반드시 준수):
- materialAnalysis.materials는 3개 또는 4개. **개수를 채우려고 약한 재료를 끼워 넣지 말 것** —
  강한 3개가 억지로 만든 4번째보다 낫다. 실제로 "방문상담 예약 서비스 도입" 같은, 주가와 사실상
  무관한 항목이 4칸을 맞추려다 들어간 사례가 있었다. 넷째 자리가 그런 수준이면 그냥 3개로 낸다.
- [실적 자체는 재료로 쓰지 말 것 — 2026-09-03 신설] "2분기 역대 최대 실적", "2분기 역대급 실적 확인"
  같은 항목은 금지한다. 분기·연간 매출과 영업이익은 이제 **DART 원본 공시 숫자로 '실적 추이' 카드가
  따로 보여준다** — 재료 슬롯에서 같은 얘기를 반복하면 칸만 낭비된다. 실적을 언급하려면 실적 그 자체가
  아니라 **그 실적을 만든 개별 사건**(특정 고객사 공급 개시, 신규 라인 가동, 판가 인상 등)을 제목으로 쓴다.
  · 나쁜 예: "2분기 역대 최대 실적", "실적 서프라이즈"
  · 좋은 예: "HBM4 ○○사 공급 개시", "광주 신규 생산라인 가동"
...
③ 일목균형표는 다루지 않는다 — 전환선·기준선·선행스팬 값이 입력 데이터에 아예 없어서, 쓰는 순간 무조건 추정이 된다(실제로 "수치가 확인되지 않는다"거나 보이지도 않는 구름대를 서술하는 사례가 반복됐다). "일목", "일목균형표", "전환선", "기준선", "구름대", "선행스팬", "후행스팬"이라는 단어 자체를 쓰지 말 것.
...
function buildSupplyFlowFact(quote) {
  ...
  return {
    ...
    // 2026-09-18: 장종료 전(투자자별 매매동향이 오늘 장중 누적치일 때) 1일 수치는
    // 마감까지 계속 바뀌는 잠정치다. 프론트가 FACT 표 위에 참고문구를 달 수 있게
    // 플래그만 실어 보낸다(문구 자체는 클라이언트가 고정 문장으로 렌더링 — AI 프롬프트에
    // 맡기면 매번 표현이 달라져 놓칠 수 있다).
    isToday: !!quote.investorFlowIsToday,
  };
}
...
async function normalizeAnalysis(raw, quote, wm, indicators) {
  ...
  let events = Array.isArray(raw.events)
    ? raw.events
        ...
        .filter((e) => e.content)
        // 사용자 규칙 — 날짜를 아예 특정할 수 없는(date 필드가 빈) 이벤트는 제외한다.
        // "2026년 하반기 예정"처럼 정확하진 않아도 시기를 특정한 표현은 date에 값이
        // 들어있으니 이 필터를 통과한다 — 완전히 빈 경우만 걸러낸다.
        .filter((e) => e.date)
        // 종목분석 시점(오늘) 이전 날짜가 명시된 이벤트는 "다가오는 이벤트"가 아니므로 제거.
        .filter((e) => !isPastEventDate(e.date, todayISO))
        // date는 있어도(혹은 AI가 미래처럼 보이는 날짜를 잘못 붙였어도) 문장 자체가
        // "~했다/~밝혔다"처럼 과거형으로 끝나고 "예정/계획" 같은 미래 표현이 없으면
        // 실제로는 이미 벌어진 일 — 재료 분석과 중복되지 않도록 여기서도 제거한다.
        .filter((e) => !isPastLeaningEventContent(e.content))
        // "오늘 기준 수급이 이렇다" 같은 현재 상황 설명은 과거형도 아니고 날짜도 채워
        // 있어서 위 필터들을 다 통과하지만 진짜 "예정된 일정"은 아니다 — 미래 지향
  ...
  // 상한 4개 — 프롬프트도 "3개 또는 4개"로 맞춰져 있다. 예전엔 프롬프트가 "최대 5개"라고
  // 하면서 서버가 4개로 자르는 불일치가 있었고, 그 탓에 모델이 개수를 맞추려 약한 재료를
  // 만들어 넣었다(2026-09-03 라이브 로그에서 확인).
...
- "기존 재료"와 "오늘의 변화"를 뒤섞어 나열하지 말고, 이 순서(과거→최근)로 자연스럽게 이어서 쓴다.`,
    ...
    "",
    `4번 다가오는 이벤트 규칙 (반드시 준수):
- upcomingEvents에는 검색으로 날짜·사실관계가 명확히 확인된 "미래" 이벤트만 포함. 이미 벌어진 일(지난 실적발표 결과 등)은 넣지 말고 5번 재료 분석에만 반영 — 이 섹션과 재료 분석이 같은 내용을 반복하면 안 된다. 애매하면 배열에서 제외(추측성 항목 금지).
- 오늘은 ${today} 이다. 오늘 날짜는 제외하고, 이 날짜보다 뒤(내일 이후) 미래 이벤트만. 2024·2025년 날짜 금지, 2026년 이후만.
- '예정/방문 예정/출시 예정/발표 예정/~할 계획/~에 참석' 키워드만. 과거형 금지.
- date가 비어 있으면 넣지 말 것 — 날짜(정확한 날짜든 '2026년 하반기 예정' 같은 대략적 시기든)를 특정할 수 없으면 제외.
- 이벤트는 '앞으로 있을 예정된 일정'이어야 한다 — 오늘/현재 시점의 상황 설명(예: '오늘 수급이 순매도라 추세가 약하다')은 이벤트가 아니라 수급 분석·재료 분석에 속한다.
...
- materialAnalysis.materials는 3개 또는 4개. 개수를 채우려고 약한 재료를 끼워 넣지 말 것 —
  강한 3개가 억지 4번째보다 낫다.
- [실적 자체 금지] "2분기 역대 최대 실적" 같은 항목은 쓰지 않는다. 매출·영업이익 추이는 DART 공시
  기반 '실적 추이' 카드가 따로 보여주므로 재료 슬롯에서 반복하면 칸 낭비다. 그 실적을 만든 개별
  사건(특정 고객사 공급 개시, 신규 라인 가동, 판가 인상 등)을 제목으로 쓸 것.
...
- 오늘은 ${today}이다. 검색으로 날짜·사실관계가 명확히 확인된, 이 날짜보다 뒤(오늘 날짜는 제외)에 아직 일어나지 않은 "미래" 일정만 포함할 것.
- 확인이 애매하거나 안 된 건은 이벤트로 넣지 말고 배열에서 제외할 것 (미확정 사실을 "아직 확인 안 됨" 같은 문구로 이벤트화하지 말 것).
- 검색 결과 이미 발표/발생한 것으로 확인되면(실적발표 포함) 그 사건은 이벤트에 넣지 말고 story/supply/materials에 결과로 반영할 것 — events와 materials 내용이 겹치면 안 된다.
- date가 없는(날짜를 특정할 수 없는) 이벤트는 넣지 말 것.
- 이벤트는 '앞으로 있을 예정된 일정'이어야 한다 — 오늘/현재 시점의 상황 설명(예: '오늘 수급이 순매도라 추세가 약하다')은 이벤트가 아니라 수급 분석·재료 분석에 속한다.
- 정말 확인된 미래 일정이 없으면 억지로 채우지 말고 빈 배열로 둘 것.
...
- MA20/60/120/200, RSI, 지지·저항 1·2차, 52주 고저는 기존과 동일하게 작성할 것.
...
- 이 섹션은 매매 "행동(수급)"만 다룬다. 자사주 매입/소각·실적·공시 같은 "재료·이벤트"를 반영률(%)로 환산해 말하지 않는다 — 그건 5번 재료 분석(materials)의 몫이다.`
      ...
      "",
      `events(다가오는 이벤트) 작성 규칙 — 반드시 준수:
- 오늘은 ${today}이다. 뉴스로 날짜·사실관계가 명확히 확인된, 이 날짜보다 뒤(오늘 날짜는 제외)에 아직 일어나지 않은 "미래" 일정만 포함할 것.
- 확인이 애매하거나 안 된 건은 이벤트로 넣지 말고 배열에서 제외할 것.
- 이미 지나간 날짜, 이미 벌어진 사건('했다/밝혔다/기록했다/하락했다' 등 과거형 서술)은 절대 이벤트로 넣지 말 것 — 그런 내용은 materials(재료 분석)에만 반영하고 events와 겹치지 않게 할 것.
- 뉴스 헤드라인은 사건 발생 시점을 참고하는 용도로만 쓰고, 과거 사건 자체를 이벤트로 재작성하지 말 것.
- date가 없는 이벤트는 넣지 말 것.
- 이벤트는 '앞으로 있을 예정된 일정'이어야 한다 — 오늘/현재 시점의 상황 설명(예: '오늘 수급이 순매도라 추세가 약하다')은 이벤트가 아니라 수급 분석·재료 분석에 속한다.
- 정말 확인된 미래 일정이 없으면 억지로 채우지 말고 빈 배열로 둘 것.
- 정확한 날짜를 모르면 '2026-07-15' 같은 구체적 날짜 대신 '2026년 하반기 예정'처럼 모호하게 표기.`,
      "",
