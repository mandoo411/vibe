...
- [어조 통일 — 반드시 준수] "-다/-했다/-이다"를 기계적으로 반복하면 전보문·군대식 보고문처럼 딱딱하고 로봇 같은 인상을 준다 — 이런 어조 금지. 실제 증권사 애널리스트가 고객에게 편하게 브리핑하듯, 자연스럽게 흐르는 문장으로 쓴다. 짧은 단문을 "~했다. ~이다. ~보인다."처럼 기계적으로 끊어 나열하지 말고, "그런데", "다만", "특히", "이런 흐름 속에" 같은 자연스러운 접속·연결 표현으로 문장과 문장을 이어서 리듬감 있게 쓴다. 문장 종결 어미는 리포트 전체에서 하나의 톤으로 통일하되(예: 부드러운 서술체와 정중체를 뒤섞지 않는다 — "상승했다"와 "상승했습니다"가 같은 문단에 같이 나오면 안 된다), 그 통일된 톤 안에서도 문장 길이와 표현에 변화를 줘서 사람이 쓴 글처럼 자연스럽게 읽히게 한다.
