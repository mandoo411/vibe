 ...
 * 선택: KIS_FLUCTUATION_MAX_PAGES, KIS_MARKET_CAP_MAX_PAGES, KIS_VOLUME_RANK_MAX_PAGES 또는 KIS_TRADE_AMOUNT_RANK_MAX_PAGES (거래량순위 연속조회 페이지 상한, 기본 12, 최대 30)
 * 선택: KIS_VOLUME_RANK_BLNG_CLS (거래량순위 소속구분 0~4, 기본 0=평균거래량), KIS_VOLUME_RANK_EXLS_CLS 또는 KIS_TRADE_VALUE_EXLS_CLS (ETF·ETN 제외 10자리, 기본 0000000011)
 */
...
/** domestic-stock/v1/quotations/volume-rank 연속조회 (기본 12페이지, 최대 30) */
const VOLUME_RANK_MAX_PAGES = Math.max(
  1,
  Math.min(
    30,
    Number(process.env.KIS_VOLUME_RANK_MAX_PAGES || process.env.KIS_TRADE_AMOUNT_RANK_MAX_PAGES) || 12
  )
);
...
function parseVolSortKey(volStr) {
  const n = Number(String(volStr || "").replace(/,/g, ""));
...
/**
 * [국내주식-047] 거래량순위 상위 50 — `volume-rank` + 연속조회.
 * fid_blng_cls_code: 0 평균거래량, 1 거래증가율, 2 평균거래회전율, 3 거래금액순, 4 평균거래금액회전율 (KIS 샘플).
 * 기본 0(거래량 기준). ETF·ETN 제외 후 누적거래량 내림차순으로 재정렬해 50건.
 */
async function fetchDomesticVolumeTop50() {
  const fidTrgtExlsClsCode = String(
    process.env.KIS_VOLUME_RANK_EXLS_CLS || process.env.KIS_TRADE_VALUE_EXLS_CLS || "0000000011"
  ).trim();
  const safeExls =
  ...
  const fidBlng = String(process.env.KIS_VOLUME_RANK_BLNG_CLS || "0").trim();
  if (!/^[01234]$/.test(fidBlng)) {
    throw new Error(`Invalid KIS_VOLUME_RANK_BLNG_CLS: ${fidBlng}`);
  }
  ...
  const rawAll = [];
  let trCont = "";
  for (let page = 0; page < VOLUME_RANK_MAX_PAGES; page++) {
    const { json, trCont: nextTr } = await kisGet(
  ...
  for (const row of rawAll) {
    ...
    if (!code || seen.has(code)) continue;
    seen.add(code);
    const name = pickHtsStockName(row) || sanitizeStr(row.hts_kor_isnm);
    if (!name || rowIsEtfOrEtn(row, name)) continue;
  ...
  candidates.sort((a, b) => parseVolSortKey(b.volume) - parseVolSortKey(a.volume));

  console.log("[kis-realtime-data][volume-rank-top50]", {
    tr_id: "FHPST01710000",
    fid_blng_cls_code: `${fidBlng} (${blngLabel})`,
    fid_trgt_exls_cls_code: safeExls,
    rawRows: rawAll.length,
    candidates: candidates.length,
    maxPagesCap: VOLUME_RANK_MAX_PAGES,
  });
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === "trade-value-top50") {
      try {
        const stocks = await fetchDomesticVolumeTop50();
        json(res, 200, { stocks });
      } catch (e) {
    ...
    if (action === "snapshot") {
      ...
      let tradeValueTop50 = [];
      try {
        tradeValueTop50 = await fetchDomesticVolumeTop50();
      } catch (e) {
