...
function financialTrendPromptBlock(fin) {
  ...
  if (fin.ttm && (fin.ttm.per != null || fin.ttm.perOp != null)) {
    const t = fin.ttm;
    lines.push(
      `최근 4개 분기 합계(${t.basis}): 순이익 ${fmtEokKo(t.netProfit)} · 영업이익 ${fmtEokKo(t.operatingProfit)} → 이 기준 PER ${t.per != null ? t.per + "배" : "—"}(영업이익 기준 ${t.perOp != null ? t.perOp + "배" : "—"})`,
      `[PER 사용 규칙 — 반드시 준수] 시세 화면의 PER${t.quotePer != null ? " " + t.quotePer + "배" : ""}는 직전 연간 실적 기준이라 실적이 급변한 종목에선 현실과 어긋난다. 밸류에이션 부담을 말할 때는 위 최근 4개 분기 기준 PER을 쓰고, 두 숫자가 크게 다르면 '직전 연간 기준 ○배, 최근 4개 분기 기준 ○배'로 둘 다 밝힌다. 순이익에 본업 밖 이익이 크게 섞였으면 영업이익 기준 배수도 함께 본다.`
    );
  }
...
/** 2026-09-26 리뷰("PER 31.6배는 옛날 실적 기준, 최근 4개 분기로 보면 8배대"):
 * 최근 4개 분기 합계(TTM) = 직전 연간 + 올해 누적 − 작년 같은 기간 누적. 누적 보고서일 때만 계산한다. */
function computeTtm(fin) {
  const q = fin && fin.quarter;
  const ann = fin && Array.isArray(fin.annual) ? fin.annual : [];
  if (!q || !q.current || !q.previous) return null;
  const ytd = q.cumulative === true || /1분기/.test(q.label || "");
  if (!ytd) return null;
  const year = parseInt(q.label, 10);
  const prevAnn = ann.find((p) => String(p.label) === String(year - 1));
  if (!prevAnn) return null;
  const pick = (k) =>
    prevAnn[k] != null && q.current[k] != null && q.previous[k] != null ? prevAnn[k] + q.current[k] - q.previous[k] : null;
  const out = { netProfit: pick("netProfit"), operatingProfit: pick("operatingProfit"), revenue: pick("revenue"), basis: `${year - 1}년 연간 + ${q.label} − ${q.prevLabel}` };
  return out.netProfit != null || out.operatingProfit != null ? out : null;
}

module.exports = { fetchFinancialTrend, financialTrendPromptBlock, computeTtm };
