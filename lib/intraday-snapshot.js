...
function buildIntradaySnapshot(prev, today, flow) {
  ...
  // 2026-09-11: 종가베팅 점수용 오늘 봉 원본값. buildIntradaySnapshot은 close/high만 쓰고
  // 있었지만, 종가위치와 윗꼬리 비율은 시가·저가가 있어야 계산된다.
  snap.openCur = num(today.open);
  snap.highCur = high;
  snap.lowCur = num(today.low);
  snap.volumeCur = volume;
  ...
  const f = flow || {};
  // 2026-09-09: 수급은 당일 집계가 미확정이면 직전 영업일 확정치가 온다(lib/kis-indicators.js
  // fetchInvestorFlow 참고). 어느 날짜 기준인지 스냅샷에 남겨 화면이 정확히 표기하게 한다.
  // KIS는 YYYYMMDD로 주지만 화면·캐시가 쓰는 표기는 YYYY-MM-DD라 여기서 맞춰 둔다.
  const fd = f.flowAsOfDate != null ? String(f.flowAsOfDate) : "";
  snap.flowAsOfDate = /^\d{8}$/.test(fd) ? `${fd.slice(0, 4)}-${fd.slice(4, 6)}-${fd.slice(6, 8)}` : null;
