...
/** 2026-09-03: PER/PBR/EPS 전용 숫자 변환. KIS가 "값 없음"을 0으로 내려주므로 0은 null로
 * 바꾼다(위 주석 참고). 적자기업의 음수 PER/EPS는 실제 값이므로 그대로 살린다. */
function toValuation(v) {
  const n = toNum(v);
  return n === 0 ? null : n;
}
...
async function fetchMarketSnapshot(code6) {
  try {
    ...
    // 2026-08-26: 매매시그널 HTS급 확장 — 위에서 이미 받아온 같은 응답(o)에 PER/PBR/EPS/
    // 외국인보유율/거래량회전율/52주 공식 고저가 필드가 그대로 들어있어서, 추가 API 호출
    // 없이 그냥 더 추출하기만 하면 된다(기존에는 marketCapEok/tradingValueEok만 썼음).
    // 2026-08-26: 사용자 제보("검색결과에 거래중지 종목도 나온다") — inquire-price 응답에
    // 이미 들어있는 temp_stop_yn(임시정지여부)/sltr_yn(정리매매여부, 상장폐지 직전 마지막
    // 거래 단계)도 추가 API 호출 없이 같이 뽑아서 즉시검색에서 걸러낼 수 있게 한다.
    return {
      ...
      // 2026-09-03: KIS는 값이 없는 종목(ETF·리츠·신규상장 등)에 대해 PER/PBR/EPS를 null이
      // 아니라 문자열 "0"으로 내려준다. 그대로 저장하면 "PBR 0.5 미만" 같은 저평가 검색이
      // 데이터 없는 종목 292개(2026-09-02 캐시 실측)를 전부 잡아버린다 — 실제로 리노공업
      // (PBR 5배대)이 "PBR 0.5 미만"에 걸리는 걸 근거 표시 기능 검증 중에 발견했다.
      // 0은 "값 없음"으로 간주해 null로 저장하고, 조건 평가에서 자연히 제외되게 한다.
      per: toValuation(o.per),
      pbr: toValuation(o.pbr),
      eps: toValuation(o.eps),
      foreignHoldRate: toNum(o.hts_frgn_ehrt),
      volTurnoverRate: toNum(o.vol_tnrt),
      high52w: toNum(o.w52_hgpr),
      low52w: toNum(o.w52_lwpr),
      // 2026-09-08: 장중 종가 스캔(scripts/intraday-close-scan.mjs)이 "오늘 봉"을 만들려면
      // 현재가·고가·저가·누적거래량이 필요하다. 같은 응답에 이미 들어 있어 추가 호출은 없다.
      close: toNum(o.stck_prpr),
      high: toNum(o.stck_hgpr),
      low: toNum(o.stck_lwpr),
      // 2026-09-11: 종가베팅 점수(lib/close-betting-score.js)의 캔들 모양 판정에 시가가
      // 필요하다. 같은 응답에 이미 들어 있어 추가 호출은 없다.
      open: toNum(o.stck_oprc),
      volume: toNum(o.acml_vol),
      tempStopYn: sanitizeStr(o.temp_stop_yn) === "Y",
      settlementTradeYn: sanitizeStr(o.sltr_yn) === "Y",
      // 2026-09-03: AI 종목분석의 "동종업계 대비 위치" 계산용. inquire-price 응답에 이미
      // 들어있는 업종 한글명이라 추가 API 호출이 없다. 값이 없는 종목(ETF 등)은 null.
      sector: sanitizeStr(o.bstp_kor_isnm) || null,
    };
  } catch {
    return {
      marketCapEok: null,
      tradingValueEok: null,
      per: null,
      pbr: null,
      eps: null,
...
/**
 * 2026-08-26: 매매시그널에 "외국인 순매수", "기관 순매수" 조건을 추가하려면 inquire-price에는
 * 없는 별도 엔드포인트(inquire-investor, FHKST01010900)를 종목당 1회 추가 호출해야 한다.
 * api/analyze.js의 fetchKisInvestorFlow(AI 종목분석 기능용)와 동일한 패턴 — 응답 output 배열의
 * 첫 행(가장 최근 영업일)에서 외국인/기관 순매수 수량만 뽑는다.
 *
 * 2026-09-08 확장(종가매매 프리셋): 이 엔드포인트는 원래 최근 여러 영업일치를 배열로 돌려주는데
 * 지금까지 첫 행만 쓰고 나머지를 통째로 버리고 있었다. "외국인·기관이 며칠째 연속으로 사고
 * 있는가"(양매수 지속)는 종가매매·수급매매 전략의 핵심 축인데 1일치만으로는 표현이 불가능했다.
 * 이미 받아온 응답을 더 읽는 것뿐이라 **KIS 추가 호출은 0회**다.
 *  - 연속 순매수 일수: 최신일부터 거슬러 올라가며 순매수(>0)가 끊기지 않은 날 수
 *  - 양매수(major) 연속 일수: 외국인·기관이 같은 날 둘 다 순매수인 날이 이어진 수
 *  - 순매수 금액(억원): frgn_ntby_tr_pbmn 등 거래대금 필드는 백만원 단위 → /100 하면 억원
 *    (api/stock-analysis.js가 같은 엔드포인트에서 이미 쓰고 있는 환산과 동일)
 * 응답 행 순서를 신뢰하지 않고 영업일(stck_bsop_date) 내림차순으로 직접 정렬한다.
 */
const INVESTOR_STREAK_MAX_DAYS = 30; // 응답이 길어도 이 정도면 충분(연속일수 상한도 겸함)

function investorRowDate(row) {
  const d = row && (row.stck_bsop_date || row.STCK_BSOP_DATE || row.bsop_date || row.date);
  const s = d == null ? "" : String(d).trim();
  return /^\d{8}$/.test(s) ? Number(s) : null;
}

async function fetchInvestorFlow(code6) {
  const empty = {
    flowAsOfDate: null,
    foreignNetBuy: null,
    institutionNetBuy: null,
    foreignNetBuyEok: null,
    institutionNetBuyEok: null,
    foreignNetBuyStreak: null,
    institutionNetBuyStreak: null,
    majorNetBuyStreak: null,
  };
  try {
    const j = await kisGetJson("/uapi/domestic-stock/v1/quotations/inquire-investor", "FHKST01010900", {
      FID_COND_MRKT_DIV_CODE: "J",
      FID_INPUT_ISCD: code6,
    });
    ...
    // 날짜가 있으면 내림차순 정렬(최신 우선), 없으면 응답 순서를 그대로 신뢰한다.
    const dated = rows.map((r, idx) => ({ r, d: investorRowDate(r), idx }));
    const hasDate = dated.some((x) => x.d != null);
    const sorted = hasDate
      ? dated.filter((x) => x.d != null).sort((a, b) => b.d - a.d).map((x) => x.r)
      : rows.slice();

    // 2026-09-09: 장중·장 마감 직후(14:30, 15:31)에는 KIS가 "당일" 행을 내려주지만 순매수
    // 수량/금액 칸이 비어 있다(집계 미확정). 예전엔 그 빈 행을 그대로 latest로 잡아
    // foreignNetBuy/institutionNetBuy가 null, 연속일수는 0이 되면서 수급을 쓰는 매매기법
    // 프리셋 3종이 전부 0개로 나왔다(실측: 2026-09-09 15:31 회차 398종목 전부 null).
    // 그래서 실제로 값이 들어 있는 가장 최근 행을 기준일로 잡는다 — 없는 값을 지어내지
    // 않으면서, 확정된 직전 영업일 수급으로 판정한다(전일 종가 캐시와 동일한 기준).
    // flowAsOfDate로 어느 날짜 수급인지 함께 돌려주어 화면이 정확히 표기할 수 있게 한다.
    const hasFlowValue = (row) =>
      toNum(row && row.frgn_ntby_qty) != null || toNum(row && row.orgn_ntby_qty) != null;
    const firstWithValue = sorted.findIndex(hasFlowValue);
    if (firstWithValue < 0) return empty;
    const usable = sorted.slice(firstWithValue);

    const latest = usable[0] || {};
    const toEok = (v) => (v == null ? null : Math.round((v / 100) * 10) / 10); // 백만원 → 억원(소수 1자리)

    /** 최신일부터 pick(row)가 양수인 날이 연속으로 몇 일 이어지는지. 값이 없으면 거기서 끊는다. */
    const streakOf = (pick) => {
      let n = 0;
      for (const row of usable.slice(0, INVESTOR_STREAK_MAX_DAYS)) {
        const v = pick(row);
        if (v == null || !(v > 0)) break;
        n += 1;
      }
    ...
    const frgnQty = (row) => toNum(row.frgn_ntby_qty);
    const orgnQty = (row) => toNum(row.orgn_ntby_qty);

    return {
      flowAsOfDate: investorRowDate(latest),
      foreignNetBuy: toNum(latest.frgn_ntby_qty),
      institutionNetBuy: toNum(latest.orgn_ntby_qty),
      foreignNetBuyEok: toEok(toNum(latest.frgn_ntby_tr_pbmn)),
      institutionNetBuyEok: toEok(toNum(latest.orgn_ntby_tr_pbmn)),
      foreignNetBuyStreak: streakOf(frgnQty),
      institutionNetBuyStreak: streakOf(orgnQty),
      majorNetBuyStreak: streakOf((row) => {
        const f = frgnQty(row);
        const o = orgnQty(row);
...
/**
 * 2026-07-19: 매매시그널 스크리닝에 "최근 3개월 안에 20% 이상 상승" 같은 기간 수익률 조건이
 * 없어서(당일 등락률만 있었음) 표준 기간(1주/1개월/3개월/6개월/1년)에 대해 "N거래일 전 종가 대비
 * 현재 종가 등락률"을 미리 계산해 둔다. 영업일 기준 근사치: 1개월≈21일, 3개월≈63일, 6개월≈126일,
 * 1년≈252일 (실제 캘린더 개월수와 정확히 일치하진 않지만 스크리닝 목적으로는 충분).
 */
const PERIOD_RETURN_DAYS = [5, 21, 63, 126, 252];

// 2026-08-26: 사용자 제보("감자나 액면병합 같은 이슈로 가격이 높아지는 것도 걸러줘. 예: 감자전
// 100원 감자후 1000원") — 한국 증시는 코스피/코스닥 모두 전일 종가 대비 ±30%가 법정 가격제한폭
// 이라(상한가/하한가), 정상적인 매매만으로는 하루 만에 종가가 30%를 넘게 움직일 수 없다.
// (참고: inquire-daily-itemchartprice 호출 시 FID_ORG_ADJ_PRC=0(수정주가)을 이미 쓰고 있어
// 유무상증자·배당락 등은 KIS가 자체 보정해주지만, 감자/주식병합처럼 상장주식수 자체가
// 바뀌는 이벤트는 "수정주가"로도 보정되지 않고 종가 시계열에 그대로 단절로 남는 것으로 보임 —
// 실제로 이 문제가 리포트된 뒤 확인한 정렬 결과 상위권에 "1년 +2900%" 같은 비정상적으로
// 딱 떨어지는 수치가 다수 섞여 있었음.) 그래서 하루 만에 종가가 이 한계를 벗어난 지점이
// 구간 안에 있으면 그 period_return은 "감자/병합/분할 등으로 단절된 시계열이라 신뢰 불가"로
// 보고 null 처리한다(원칙: 실데이터 신뢰 못 하면 지어내지 않고 표시 생략).
const ANOMALY_JUMP_THRESHOLD = 0.32; // 코스피/코스닥 법정 가격제한폭(±30%)에 반올림 오차 여유를 더한 컷오프

function findAnomalyDayIndices(closes) {
  const anomalyAfter = new Array(closes.length).fill(false);
  for (let i = 1; i < closes.length; i++) {
    const prev = closes[i - 1];
    const cur = closes[i];
    if (prev == null || cur == null || prev === 0) continue;
    if (Math.abs((cur - prev) / prev) > ANOMALY_JUMP_THRESHOLD) anomalyAfter[i] = true;
  }
  return anomalyAfter;
}

/**
 * 2026-09-26: "1년(252)" 기준점을 거래일 252개 전이 아니라 **달력 52주(364일) 창의 첫 거래일**로 잡는다.
 * 한국 증시는 1년에 245~248거래일이라 252거래일 전은 52주보다 1~2주 더 과거였다 — SK하이닉스
 * 리포트에서 "1년 +512.8%"의 기준가(30.7만)가 52주 저점(33.3만)보다 낮게 나오는 모순이 생겼다.
 * KIS 52주 고저와 같은 창을 쓰면 기준가는 항상 52주 고저 범위 안에 들어간다.
 * times가 없으면(옛 호출부) 예전처럼 거래일 수로 계산한다.
 */
function ymdToUtcMs(t) {
  const m = String(t || "").match(/^(\d{4})-?(\d{2})-?(\d{2})/);
  return m ? Date.UTC(+m[1], +m[2] - 1, +m[3]) : null;
}

function yearBaseIndex(times, endIdx, shiftDays) {
  if (!Array.isArray(times) || endIdx < 0 || times[endIdx] == null) return null;
  const endMs = ymdToUtcMs(times[endIdx]);
  const firstMs = ymdToUtcMs(times[0]);
  if (endMs == null || firstMs == null) return null;
  const cutoff = endMs - (364 - (shiftDays || 0)) * 86400000;
  if (firstMs > cutoff) return -1; // 1년치 시계열이 없다
  for (let i = 0; i <= endIdx; i++) {
    const ms = ymdToUtcMs(times[i]);
    if (ms != null && ms >= cutoff) return i;
  }
  return -1;
}

function computePeriodReturns(closes, times) {
  const n = closes.length;
  const anomalyAfter = findAnomalyDayIndices(closes);
  const out = {};
  for (const days of PERIOD_RETURN_DAYS) {
    let idx = n - 1 - days;
    if (days === 252 && Array.isArray(times) && times.length === n) {
      const yi = yearBaseIndex(times, n - 1, 0);
      if (yi != null) idx = yi;
    }
    if (idx < 0 || closes[idx] == null || closes[idx] === 0 || closes[n - 1] == null) {
      out[days] = null;
      continue;
    }
    let anomalous = false;
    ...
    out[days] = anomalous ? null : Math.round(((closes[n - 1] - closes[idx]) / closes[idx]) * 10000) / 100;
  }
  return out;
}
