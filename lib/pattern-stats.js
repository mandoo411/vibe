...
function baseProbLines(ps) {
  ...
  return [
    ...
    "시나리오 확률(probability)은 반드시 이 기준 확률에서 출발한다. 수급·재료·밸류에이션으로 조정하되, 어느 시나리오든 기준과 10%p 넘게 다르게 매기면 그 시나리오 basis 첫 문장에 '기준 40%→최종 28%: 이유' 형식으로 조정 이유를 적는다. 조정 폭은 시나리오당 10%p를 넘기지 않는다. 화면에 기준 확률과 최종 확률이 나란히 표시된다.",
    "확률 근거(basis)에서 기준 확률 숫자로 자기 자신을 설명하지 않는다(금지 예: '기준 61%에서 크게 벗어나지 않습니다', '기준 8%가 맞습니다', '기준 31% 수준이 가장 자연스럽습니다'). 대신 기준에서 올리거나 내린(또는 그대로 둔) 이유를 수급·재료·추세 사실로 쓴다.",
    "매매 계획은 20거래일 기준이므로 과거 통계는 20거래일 뒤 수치를 인용한다(5거래일 수치는 근거로 쓰지 않는다). 20거래일 뒤 단순 상승 비율과 강세(A) 확률은 정의가 다르다(A는 +" + b.band + "% 이상 오른 경우만) — 둘이 크게 다르면 그 차이를 한 번 짚는다.",
  ];
}
...
function patternStatsPromptBlock(ps) {
  ...
  return [
    ...
    ...baseProbLines(ps),
    ps.sample < 30 ? `표본이 ${ps.sample}회로 적으므로 통계를 인용할 때 '표본이 적어 참고용'이라는 점을 한 번 밝힌다.` : "",
    "확률 근거(basis)에는 이 통계를 '과거 비슷한 국면 N회 중 20거래일 뒤 M회 상승'처럼 한 번 인용한다. 이 통계에 없는 표본 수·적중률을 새로 만들지 않는다. 화면에도 같은 표가 따로 나가니 숫자를 길게 반복하지 않는다.",
  ].join("\n");
}
