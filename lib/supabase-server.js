...
/** 2026-09-26: 분석이 실패하면 방금 올린 무료 사용횟수를 1 되돌린다(0 미만 금지). 실패해도 조용히 넘어간다. */
async function refundFreeUsage(userId, monthKey) {
  if (!userId || !monthKey || !isConfigured()) return false;
  try {
    const q = `user_id=eq.${encodeURIComponent(userId)}&month=eq.${encodeURIComponent(monthKey)}`;
    const h = { apikey: serviceRoleKey(), Authorization: `Bearer ${serviceRoleKey()}`, "content-type": "application/json" };
    const r = await fetch(`${supabaseUrl()}/rest/v1/analysis_usage?${q}&select=count`, { headers: h });
    if (!r.ok) return false;
    const rows = await r.json();
    const cur = rows && rows[0] ? Number(rows[0].count) || 0 : 0;
    if (cur <= 0) return false;
    const u = await fetch(`${supabaseUrl()}/rest/v1/analysis_usage?${q}`, {
      method: "PATCH",
      headers: { ...h, Prefer: "return=minimal" },
      body: JSON.stringify({ count: cur - 1, updated_at: new Date().toISOString() }),
    });
    return u.ok;
  } catch (e) {
    console.error("[supabase-server] refundFreeUsage 실패", e && e.message);
    return false;
  }
}
