...
function computeTechExtras(candles) {
  ...
  return out.macd || out.bollinger || out.volume ? out : null;
}
...
function techExtrasPromptBlock(tx, fmtPrice) {
  ...
  lines.push(
    "이 블록을 차트 흐름 분석에 반영한다: RSI 항목 바로 다음 줄에 '보조지표 교차 확인: …' 한 줄을 추가해, RSI·MACD·볼린저·거래량이 같은 방향을 가리키는지 엇갈리는지를 판단하고 엇갈리면 어느 쪽을 더 믿는지와 이유를 쓴다. 숫자는 두 개까지만 인용한다. 이 블록에 없는 지표 값은 만들지 않는다."
  );
  return lines.join("\n");
}
