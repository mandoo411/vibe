...
const ALLOWED_CLAUSE_TYPES = [
  ...
  "foreign_net_buy",
  "institution_net_buy",
  // 2026-09-08: 종가매매 프리셋 — 수급의 "지속성"과 "규모"를 표현하기 위한 확장
...
- volume_ratio: {op:"gte"|"gt"|"lte"|"lt", value: 숫자(%)} — 20일 평균 거래량 대비 당일 거래량
  비율(%). 100이 평소 수준이고 200이면 평소의 2배. "거래량이 평소보다 줄어든/말라붙은 종목"
  처럼 감소를 뜻하면 op:"lte"(예: value:70)를 쓸 것
...
- foreign_net_buy_amount: {op:"lt"|"lte"|"gt"|"gte", value:숫자(억원)} — 외국인 당일 순매수 "금액".
  "외국인이 100억 이상 순매수"면 value:100, op:"gte". 수량(주)이 아니라 금액을 말한 경우에만 쓸 것
- institution_net_buy_amount: {op, value:숫자(억원)} — 기관 당일 순매수 금액
- foreign_net_buy_streak: {op:"gte"|"gt"|"lte"|"lt", value:숫자(일)} — 외국인이 오늘 포함 며칠째
  연속 순매수인지. "외국인 3일 연속 순매수"면 op:"gte", value:3 (0이면 오늘 순매수가 아니라는 뜻)
- institution_net_buy_streak: {op, value:숫자(일)} — 기관 연속 순매수 일수
- major_net_buy_streak: {op, value:숫자(일)} — 외국인·기관이 **같은 날 둘 다** 순매수한 날이
  며칠째 이어지는지("양매수" 지속일수). "외국인 기관 동반 순매수 3일 연속"이면 op:"gte", value:3.
  "외국인 기관 양매수"처럼 일수가 없으면 op:"gte", value:1
...
function buildSnapshotFromSeries(series) {
  const {
    closes, highs, lows, opens, volumes, ma, rsiSeries, divergence, candles, macd, bollinger, stochastic, adx,
    marketCapEok, tradingValueEok, periodReturns,
    per, pbr, eps, foreignHoldRate, volTurnoverRate, foreignNetBuy, institutionNetBuy,
    foreignNetBuyEok, institutionNetBuyEok,
    foreignNetBuyStreak, institutionNetBuyStreak, majorNetBuyStreak,
  ...
  const snap = {
    ...
    /* 2026-09-11: 종가베팅 점수가 종가위치 (종가-저가)/(고가-저가)와 윗꼬리 비율을 쓴다.
     * 파생지표만 저장하던 탓에 원본 봉 값이 스냅샷에 없었다. opens는 구버전 호출부에서
     * 안 넘어올 수 있어 옵셔널로 둔다. */
    openCur: opens && opens[i] != null ? opens[i] : null,
    highCur: highs && highs[i] != null ? highs[i] : null,
    lowCur: lows && lows[i] != null ? lows[i] : null,
    volumeCur: volumes && volumes[i] != null ? volumes[i] : null,
  };
  ...
  /* ────────────────────────────────────────────────────────────────────────
   * 2026-09-08: 장중 종가 스캔용 "다음 봉" 기준값 (snap.next)
   *
   * 장 마감 직후 15:31에 종가매매 후보를 뽑으려면 당일 봉이 반영된 지표가 필요한데,
   * 전종목 일봉을 다시 받아오면 80분이 걸려 15:40 마감을 절대 못 맞춘다. 그래서
   * "어제까지의 시계열에 오늘 봉 하나만 붙였을 때"의 계산에 필요한 값을 미리 접어둔다.
   * 이 값들만 있으면 오늘 종가·고가·거래량 3개로 MA·이격도·거래량비·기간수익률·
   * 신고가 판정을 KIS 추가 호출 없이 정확히 다시 계산할 수 있다(lib/intraday-snapshot.js).
   *
   * 이름의 Next는 전부 "새 봉이 추가됐을 때 쓰일 값"이라는 뜻이다:
   *  - avgVol20: 새 봉의 20일 평균 거래량(= 오늘 포함 직전 20봉 평균). 지표 정의상
   *    당일 거래량은 분모에서 빠지므로 새 봉 없이 지금 확정할 수 있다.
   *  - maTailDrop[p]: 새 봉이 들어올 때 p일 이동평균에서 빠져나갈 가장 오래된 종가
   *  - periodBase[d]: 새 봉 기준 d거래일 전 종가(기간수익률 분모)
   *  - high252 / highWindow[w]: 새 봉의 신고가 판정에 쓸 직전 구간 최고가
   * ──────────────────────────────────────────────────────────────────────── */
  const nextVolWindow = (volumes || []).slice(Math.max(0, i - 19), i + 1);
  const nextHigh = (w) => {
    const arr = (highs || []).slice(Math.max(0, i - w + 1), i + 1);
    return arr.length ? Math.max(...arr) : null;
  };
  snap.next = {
    avgVol20: nextVolWindow.length ? nextVolWindow.reduce((a, b) => a + b, 0) / nextVolWindow.length : null,
    maTailDrop: {},
    periodBase: {},
    high252: nextHigh(252),
    highWindow: {},
    volWindow: {},
  };
  for (const p of MA_PERIODS) {
    const idx = i - p + 1;
    snap.next.maTailDrop[p] = idx >= 0 ? closes[idx] : null;
  }
  for (const d of [5, 21, 63, 126, 252]) {
    let idx = i - d + 1;
    // 2026-09-26: 1년은 달력 52주 창 기준(kis-indicators computePeriodReturns와 동일). 새 봉은 내일이라 창을 하루 민다.
    if (d === 252 && Array.isArray(candles) && candles.length === n) {
      const yi = require("./kis-indicators.js").yearBaseIndex(candles.map((c) => c && c.time), i, 1);
      if (yi != null) idx = yi;
    }
    snap.next.periodBase[d] = idx >= 0 ? closes[idx] : null;
  }
  for (const w of WINDOW_BAR_OPTIONS) {
    snap.next.highWindow[w] = nextHigh(w);
    const vs = (volumes || []).slice(Math.max(0, i - w + 1), i + 1);
    snap.next.volWindow[w] = vs.length ? Math.max(...vs) : null;
  }
  ...
  // 2026-09-08: 종가매매/수급매매 프리셋용 확장 수급 필드(lib/kis-indicators.js fetchInvestorFlow).
  // 순매수 금액은 억원, 연속일수는 "오늘 포함 며칠째 연속 순매수인지"(0 = 오늘 순매수 아님).
  // 구버전 캐시에는 이 필드들이 아예 없어 undefined -> null이 되고, 관련 조건은 cmp에서
  // 자연히 false가 된다(값 없는 종목이 0으로 취급돼 잘못 걸리는 일 없음).
  snap.foreignNetBuyEok = foreignNetBuyEok != null ? foreignNetBuyEok : null;
  snap.institutionNetBuyEok = institutionNetBuyEok != null ? institutionNetBuyEok : null;
  snap.foreignNetBuyStreak = foreignNetBuyStreak != null ? foreignNetBuyStreak : null;
  snap.institutionNetBuyStreak = institutionNetBuyStreak != null ? institutionNetBuyStreak : null;
  snap.majorNetBuyStreak = majorNetBuyStreak != null ? majorNetBuyStreak : null;
...
function evaluateClauseOnSnapshot(clause, snap) {
  ...
  if (clause.type === "volume_ratio") {
    // 2026-09-08: 예전엔 gt/gte만 처리하고 lt/lte가 오면 조용히 gte로 취급했다 —
    // "거래량이 평소보다 줄어든 종목"(눌림목의 핵심 조건)을 요청하면 정반대인
    // "거래량 급증" 결과가 나오는 상태였다. 4개 연산자를 모두 지원한다.
    if (snap.volumeRatio == null) return false;
    if (clause.op === "lt") return snap.volumeRatio < clause.value;
    if (clause.op === "lte") return snap.volumeRatio <= clause.value;
    if (clause.op === "gt") return snap.volumeRatio > clause.value;
    return snap.volumeRatio >= clause.value;
  }
  ...
  // 2026-09-08 종가매매 프리셋용 수급 확장 --------------------------------
  if (clause.type === "foreign_net_buy_amount") return cmp(snap.foreignNetBuyEok, clause.op, clause.value);
  if (clause.type === "institution_net_buy_amount") return cmp(snap.institutionNetBuyEok, clause.op, clause.value);
  if (clause.type === "foreign_net_buy_streak") return cmp(snap.foreignNetBuyStreak, clause.op, clause.value);
  if (clause.type === "institution_net_buy_streak") return cmp(snap.institutionNetBuyStreak, clause.op, clause.value);
  if (clause.type === "major_net_buy_streak") return cmp(snap.majorNetBuyStreak, clause.op, clause.value);
...
function clauseEvidence(clause, snap) {
  ...
  if (t === "foreign_net_buy_amount") {
    const v = evSigned(snap.foreignNetBuyEok, 1, "억");
    return v == null ? null : { label: "외국인 순매수액", value: v };
  }
  if (t === "institution_net_buy_amount") {
    const v = evSigned(snap.institutionNetBuyEok, 1, "억");
    return v == null ? null : { label: "기관 순매수액", value: v };
  }
  if (t === "foreign_net_buy_streak") {
    const v = evNum(snap.foreignNetBuyStreak, 0);
    return v == null ? null : { label: "외국인 연속 순매수", value: `${v}일` };
  }
  if (t === "institution_net_buy_streak") {
    const v = evNum(snap.institutionNetBuyStreak, 0);
    return v == null ? null : { label: "기관 연속 순매수", value: `${v}일` };
  }
  if (t === "major_net_buy_streak") {
    const v = evNum(snap.majorNetBuyStreak, 0);
    return v == null ? null : { label: "외국인·기관 양매수", value: `${v}일 연속` };
  }
  if (t === "debt_ratio") {
