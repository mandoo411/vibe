...
function buildUserPrompt({
  ...
  "usd_krw": 0,
  "indexDetail": {
    "kospi":  { "open": 0, "high": 0, "low": 0 },
    "kosdaq": { "open": 0, "high": 0, "low": 0 }
  },
  "analysis": "시장 흐름 분석 텍스트",
  "investor_trend": {
    "kospi": { "foreign": "...", "institution": "...", "individual": "...", "etc": "..." },
    "kosdaq": { "foreign": "...", "institution": "...", "individual": "...", "etc": "..." }
  },
  "sectors": [
    { "name": "전기전자", "market": "KOSPI", "change_pct": 1.22 }
  ],
  "netBuyLeaders": {
    "foreign": [{ "name": "", "amount": "" }],
  ...
  "featured_stocks": [
    { "name": "", "code": "", "price": 0, "change_pct": 0, "type": "급등|급락", "reason": "", "point": "", "risk": "" }
  ],
  "stocks": [
...
[신규 구조화 필드 작성 규칙 — 화면에서 차트/표로 직접 렌더링되므로 정확도가 최우선]
- marketTone: 오늘 장의 성격을 4~6자 한 단어로. 예: "전약후강", "전강후약", "상승반전", "장중 급락", "박스권", "동반 하락". 화면 상단 태그로 노출된다.
- indexDetail: 시가/장중 고가/장중 저가. 뉴스·데이터에서 확인되는 값만 채우고, 확인 안 되는 항목은 그 키 자체를 빼라(0이나 추정치 절대 금지). 이 값들로 "장중 저점 → 종가" 레인지 바를 코드가 그린다.
- investor_trend.etc: 기타법인 순매수. 확인 안 되면 키를 생략. 3대 주체가 모두 순매도인데 지수가 오른 날처럼 기타법인이 결정적이었던 날에는 반드시 채워라.
- sectors: 업종별 등락률. 상승 상위 3~5개와 하락 상위 3~5개를 함께 담아라(한쪽만 담지 말 것). change_pct는 숫자만(% 기호 없이). 업종 등락률이 데이터·뉴스에 없으면 sectors 배열 자체를 생략하라.
- netBuyLeaders: 외국인/기관 순매수 상위 종목(각 3~5개). amount는 "+1,234억"처럼 부호+억 단위 문자열. 확인 안 되면 해당 키를 생략.
- featured_stocks[].risk: 그 종목을 지금 따라 사면 무엇이 위험한지 한 문장(40자 내외). 예: "거래대금이 테마 수급에 쏠려 있어 재료 소멸 시 되돌림이 빠르다". point(향후 전망)와 다른 내용이어야 하며, 같은 말을 바꿔 쓴 것이면 안 된다. 반드시 모든 특징주에 채워라 — 유료 구독자가 가장 원하는 정보다.`);
...
6. 향후 전략 및 총평 — 코스피/코스닥 각각 전략 (이 섹션에는 "내일 주목할 변수"를 포함하지 말 것)
   ※ "관망이 유효하다", "선별적 접근이 필요하다" 같은 누구나 쓸 수 있는 문장으로 끝내지 마라. 구독료를 내는 독자가 내일 실제로 행동할 수 있게, 최소한 다음 두 가지를 담아라 — ① 어떤 조건(지수 레벨·수급 방향·특정 지표)이 확인되면 방향을 바꿔야 하는지, ② 지금 무엇을 피해야 하는지(과열 업종·수급 이탈 구간). 오늘 데이터에서 실제로 확인된 숫자를 근거로 써라.
7. 내일 주목할 변수 — 별도의 독립 섹션으로 구성. 향후 전략 섹션과 구분선으로 분리하고, 정확히 3가지 변수를 각각 "1." "2." "3."으로 시작하는 **별개의 줄**로 제시한다.
   ※ 한 번호 안에 여러 변수를 이어 붙이지 마라(화면이 번호 단위로 카드를 나눠 그리기 때문에, 뭉쳐 쓰면 한 카드에 네 가지가 들어가 버린다). 각 항목은 2~3문장, 반드시 "무엇을 볼지 → 그래서 어떻게 해석할지" 순서로 쓴다.
