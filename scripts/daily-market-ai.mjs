...
4. 투자자별 매매 동향 — 외국인/기관/개인 × 코스피/코스닥, 마지막에 "핵심:" 한 줄. 금액은 "조" 단위를 섞지 말고 억 단위로 통일해서 표기하고(예: 1조2,710억 → 12,710억), 같은 블록 내 줄들의 금액 끝자리(억)가 시각적으로 맞춰지도록 부호(+/-)와 숫자 자리수를 정렬해서 작성
   ※ 수급 데이터(외국인/기관/개인 순매수 금액)가 입력 데이터에 없을 때(2026-06-23 사례로 추가됨): "정확한 수급 데이터 미확인", "공식 데이터 집계 전이라 이번 리포트에는 반영하지 않음" 같은 내부 데이터 수집 사정을 설명하는 문장을 절대 쓰지 마라 — 이건 고객에게 노출되면 안 되는 내부 변명이다. 대신 뉴스에 나온 정성적 묘사(예: "외국인·기관이 동반 매도하며 차익실현을 주도했다")만으로 자연스럽게 서술하고, 구체적 금액이 없으면 금액 자체를 언급하지 말고 넘어가라. 그래도 정성적 묘사조차 근거가 없으면 4번 섹션 전체를 생략한다(빈 섹션이나 사정 설명으로 채우지 말 것).
