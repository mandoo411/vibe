...
// 5) package.json 문법 오류 — 깨지면 npm ci를 쓰는 워크플로우 11개가 전부 조용히 실패한다
// (2026-08-09 사고: trailing comma 하나로 약 2시간 동안 지표 갱신/인스타 발행/장중
// 시세 동기화가 전부 실패했는데 아무도 바로 눈치채지 못했다. 재발 방지용 체크.)
function checkPackageJsonSyntax() {
    try {
          JSON.parse(readFileSync(path.resolve("package.json"), "utf8"));
    } catch (error) {
          fail(
                  "package-json-invalid",
                  `package.json이 깨진 JSON입니다 — npm ci를 쓰는 워크플로우 11개가 전부 실패 중일 수 있음: ${error.message}`
                );
    }
}
