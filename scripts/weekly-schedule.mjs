...
/* 2026-09-02: 이전 프롬프트가 analysis 문장 자체를 "예상 상회 시 …. 예상 하회 시 …."
   템플릿으로 못 박아둬서, 화면에 4건이 뜨면 네 건이 전부 같은 문형으로 읽혔다(감사 지적).
   상회/하회 라벨은 화면이 그리게 두고 AI는 '그래서 뭐가 움직이는지'만 쓰게 필드를 쪼갠다.
   why/bull/bear/watch는 weekly-market.html의 analysisItemHtml()이 렌더한다 — 필드명을 바꾸면
   그쪽도 같이 고쳐야 한다. 구버전 레코드(analysis 한 덩어리)는 화면이 폴백으로 처리한다. */
function buildAnalysisPrompt(payload) {
  return `아래 이번 주~다음 주 핵심 경제지표와 국내 실적 발표 일정을 분석해서 한국어 JSON만 반환해줘.
독자는 한국 주식·환율을 직접 매매하는 개인 투자자다.
  ...
  "eventAnalysis": [
    {
      ...
      "date": "YYYY-MM-DD",
      "title": "지표명 (약어)",
      "expected": "예측 X / 이전 Y (단위 포함, 값이 없으면 키 생략)",
      "why": "지금 이 지표를 왜 보는지 — 직전 발표와 무엇이 달라졌는지, 시장이 현재 무엇을 가격에 반영해 두고 있는지. 1~2문장.",
      "bull": "예상보다 높게 나왔을 때 실제로 움직이는 것(지수·업종·환율·금리). '예상 상회 시'로 시작하지 말 것 — 결과만 쓴다. 1~2문장.",
      "bear": "예상보다 낮게 나왔을 때 실제로 움직이는 것. '예상 하회 시'로 시작하지 말 것. 1~2문장.",
      "watch": ["함께 볼 지표·업종·종목 2~4개, 각 12자 이내"]
    },
    {
      "type": "earnings",
      "date": "YYYY-MM-DD",
      "title": "회사명 실적발표",
      "expected": "컨센서스 영업이익 X조원 / 전년동기 대비 Y% (모르면 키 생략)",
      "why": "이번 분기 실적에서 시장이 실제로 확인하려는 것 한 가지. 1~2문장.",
      "bull": "호실적일 때 같은 업종에 어떻게 번지는지. 1~2문장.",
      "bear": "부진할 때 같은 업종에 어떻게 번지는지. 1~2문장.",
      "watch": ["같이 볼 종목·지표 2~4개"]
    }
  ]
}

작성 규칙(중요):
1. "…할 가능성이 높습니다", "…할 수 있습니다", "긍정적인 영향을 미칠 수 있습니다" 같은 애매한 마무리를 쓰지 마라. 무엇이 오르고 무엇이 내리는지 방향을 특정해서 써라.
2. 6개 항목의 문장 구조가 서로 달라야 한다. 같은 문형을 두 번 이상 반복하지 마라.
3. bull/bear는 서로 단순 대칭(오른다/내린다)이 아니어야 한다. 비대칭인 지표면 그 비대칭을 써라.
4. 확인되지 않은 수치를 지어내지 마라. 예측치·이전치가 데이터에 없으면 expected 키를 생략한다.
5. 문장은 40~70자, 최대 90자. 한 문장에 절 3개 이상 넣지 마라.
6. watch에는 실제로 존재하는 지표명·업종명·상장 종목명만 넣어라.
