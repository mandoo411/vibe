(function () {
  ...
  /** AI Ï¢ÖÎ™©Î∂ÑÏÑù ?ëÍ∑º ?úÌïú (Î≤†Ì?) */
  const ANALYSIS_PAGE_LOCKED = false;
  const ANALYSIS_HREF = "./stock-analysis.html";

  const TM_ALL_PAGES = [
    { id: "home", href: "./index.html", label: "??, icon: "ti-home" },
    { id: "realtime", href: "./realtime.html", label: "?§ÏãúÍ∞ÑÏãú??, icon: "ti-activity" },
    { id: "analysis", href: "./stock-analysis.html", label: "AI Ï¢ÖÎ™©Î∂ÑÏÑù", icon: "ti-robot" },
    { id: "schedule", href: "./weekly-market.html", label: "?ºÏ†ï", icon: "ti-calendar" },
    { id: "briefing", href: "./briefing.html", label: "Î∏åÎ¶¨??, icon: "ti-file-description" },
    { id: "daily", href: "./daily-market.html", label: "ÎßàÍ∞ê?úÌô©", icon: "ti-chart-bar" },
    { id: "market", href: "./market.html", label: "?úÏû•ÏßÄ??, icon: "ti-chart-line" },
    { id: "us", href: "./us-market.html", label: "ÎØ∏Íµ≠Ï£ºÏãù", icon: "ti-building-skyscraper" },
    { id: "crypto", href: "./crypto.html", label: "?îÌò∏?îÌèê", icon: "ti-currency-bitcoin" },
    { id: "world", href: "./world-market.html", label: "Í∏ÄÎ°úÎ≤å??Çπ", icon: "ti-world" },
  ];

  /** ?ÑÏ≤¥ Î©îÎâ¥ ?úÌä∏ 3√ó3 (???∞ÏÑ†) */
  ...
  function ensureAnalysisGate() {
    ...
    gate.innerHTML =
      ...
      '<h2 id="ai-access-gate-title" class="ai-access-gate__title">?úÎπÑ??Ï§ÄÎπ?Ï§?/h2>' +
      '<p class="ai-access-gate__text">AI Ï¢ÖÎ™©Î∂ÑÏÑù?Ä ?ÑÏû¨ Î≤†Ì? ?åÏä§??Ï§ëÏûÖ?àÎã§.<br>?ïÏãù ?§Ìîà ???åÎ¶º???úÎ¶¨Í≤†Ïäµ?àÎã§.</p>' +
      '<a class="ai-access-gate__btn" href="./index.html">?àÏúºÎ°??åÏïÑÍ∞ÄÍ∏?/a>' +
      "</div>";
  ...
  function formatTickerValue(item) {
    ...
    if (/ÎπÑÌä∏ÏΩîÏù∏|BTC/i.test(label)) return `$${Math.round(value).toLocaleString("ko-KR")}`;
    if (label.includes("???¨Îü¨")) return value.toLocaleString("ko-KR", { maximumFractionDigits: 2 });
    if (label.includes("?†Í?") || label.includes("Í∏?)) return value.toLocaleString("ko-KR", { maximumFractionDigits: 2 });
    return value.toLocaleString("ko-KR", { maximumFractionDigits: 2 });
  }

  function formatTickerPct(value, label) {
    const n = Number(value);
    const isUsdKrw = String(label || "").includes("???¨Îü¨");
  ...
  function tickerPctHtml(item) {
    ...
    const isUsdKrw = label.includes("???¨Îü¨");
  ...
  function filterWebTickerItems(items) {
    ...
    return list.filter((item) => !String(item?.label || "").includes("Í∏àÏãú??));
  }
  ...
  function renderTicker() {
    ...
    fetch("/api/market-ticker?t=" + Date.now(), { cache: "no-store" })
      ...
      .then((data) => {
        ...
        if (!items.length) {
          el.innerHTML = '<span class="home-empty">?úÏû• ÏßÄ??Î°úÎî© Ï§ë‚Ä?/span>';
          return;
        }
        el.innerHTML = items
          .map((item) => {
      ...
      .catch(() => {
        el.innerHTML = '<span class="home-empty">?úÏû• ÏßÄ?úÎ? Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??/span>';
      });
  }
  ...
  function createBottomNavItem(tabId) {
    ...
    if (isMenu) {
      el.type = "button";
      el.setAttribute("aria-label", "?ÑÏ≤¥ Î©îÎâ¥");
  ...
  function ensureNavSheet() {
    ...
    sheet.innerHTML =
      ...
      '<header class="tm-nav-sheet__head"><h2 id="tm-nav-sheet-title">?ÑÏ≤¥ Î©îÎâ¥</h2>' +
      '<button type="button" class="tm-nav-sheet__close" data-close-sheet aria-label="?´Í∏∞"><i class="ti ti-x"></i></button></header>' +
  ...
  function injectMobileMeta() {
    ...
    meta.innerHTML =
      '<div class="home-nav__live" aria-label="?§ÏãúÍ∞?>' +
      '<span class="home-nav__live-dot" aria-hidden="true"></span>' +
      '<span class="home-nav__live-text">LIVE</span></div>' +
      '<button type="button" class="home-nav__theme home-nav__theme--header tm-theme-toggle" aria-label="?åÎßà ?ÑÌôò" title="?åÎßà ?ÑÌôò">' +
